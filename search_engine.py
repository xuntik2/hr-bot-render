...
Версия 4.6 — оптимизированный поиск, предложения, синонимы для IT,
поддержка обновления данных через refresh_data().
"""
...
class SearchEngine:
    ...
    SYNONYMS = {
        ...
        'it': 'айти',          # английское написание
        'айти': 'айти',       # уже нормализовано
        'ит': 'айти',         # русская аббревиатура
    ...
    def __init__(self, max_cache_size: int = 200):
        ...
        logger.info(f"✅ SearchEngine v4.6: загружено {len(self.faq_data)} записей, "
    ...
    def search(self, query: str, category: Optional[str] = None, top_k: int = 5) -> List[Tuple[str, str, float]]:
        ...
        # --- ЭТАП 2: БАЗОВАЯ ОЦЕНКА ДЛЯ КАНДИДАТОВ ---
        candidates_with_score = []
        for faq in preliminary[:20]:
            base_score = 0.0
            q_words = set(faq.norm_question.split()) if faq.norm_question else set()
            common_q = query_words.intersection(q_words)
        ...
        # --- ЭТАП 3: ПОЛНЫЙ РАСЧЁТ ДЛЯ ТОП-10 ---
        results = []
    ...
    # ------------------------------------------------------------
    #  ОБНОВЛЕНИЕ ДАННЫХ (вызывается извне при изменении faq.json)
    # ------------------------------------------------------------
    def refresh_data(self):
        """Принудительная перезагрузка данных из faq.json и сброс кэша."""
        self._load_faq()
        self._build_category_index()
        self.cache.clear()
